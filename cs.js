...
!(function() {
  ...
  function projectionTween(a, b) {
    ...
    function raw(Î», Ï†) {
      var pa = a([Î» *= 180 / Math.PI, Ï† *= 180 / Math.PI]), pb = b([Î», Ï†]);
      return [(1 - Î±) * pa[0] + Î± * pb[0], (Î± - 1) * pa[1] - Î± * pb[1]];
    }
  
    prj.alpha = function(_) {
      if (!arguments.length) return Î±;
      Î± = +_;
      var ca = a.center(), cb = b.center(),
          ta = a.translate(), tb = b.translate();
      
      center([(1 - Î±) * ca[0] + Î± * cb[0], (1 - Î±) * ca[1] + Î± * cb[1]]);
      translate([(1 - Î±) * ta[0] + Î± * tb[0], (1 - Î±) * ta[1] + Î± * tb[1]]);
      return prj;
    };
  ...
  function transform(c, euler) {
    var x, y, z, Î², Î³, Î», Ï†, dÏˆ, Ïˆ, Î¸,
        Îµ = 1.0e-5;
  
    if (!euler) return c; 
  
    Î» = c[0];  // celestial longitude 0..2pi
    if (Î» < 0) Î» += Ï„; 
    Ï† = c[1];  // celestial latitude  -pi/2..pi/2
    
    Î» -= euler[0];  // celestial longitude - celestial coordinates of the native pole
    Î² = euler[1];  // inclination between the poles (colatitude)
    Î³ = euler[2];  // native coordinates of the celestial pole
    
    x = Math.sin(Ï†) * Math.sin(Î²) - Math.cos(Ï†) * Math.cos(Î²) * Math.cos(Î»);
    if (Math.abs(x) < Îµ) {
      x = -Math.cos(Ï† + Î²) + Math.cos(Ï†) * Math.cos(Î²) * (1 - Math.cos(Î»));
    }
    y = -Math.cos(Ï†) * Math.sin(Î»);
    ...
    if (Î» % Math.PI === 0) {
      Î¸ = Ï† + Math.cos(Î») * Î²;
      if (Î¸ > halfÏ€) Î¸ = Math.PI - Î¸; 
      if (Î¸ < -halfÏ€) Î¸ = -Math.PI - Î¸; 
    } else {
      z = Math.sin(Ï†) * Math.cos(Î²) + Math.cos(Ï†) * Math.sin(Î²) * Math.cos(Î»);
      if (Math.abs(z) > 0.99) {
        Î¸ = Math.abs(Math.acos(Math.sqrt(x*x+y*y)));
        if (z < 0) Î¸ *= -1; 
      } else {
        Î¸ = Math.asin(z);
      }
  ...
  //Defaults
  var settings = { 
    width: 1080,     // Default width; height is determined by projection
    ...
    timezoneid: "WZ9LXQ27HZ5F", // Account ID for TimeZoneDB service, please get your own
    ...
    stars: {
      ...
      size: 12,       // Scale size (radius) of star circle in pixels
      exponent: -0.29,  // Scale exponent for star size, larger = more linear
    ...
    lines: {
      graticule: { show: false, stroke: "#949494", width: 2.8, opacity: 0.7,      // Show graticule lines
    ...
    planets: {  //Show planet locations, if date-time is set
      ...
      symbols: {
        ...
        "lun": {symbol: "\u25cf", letter:"L", fill: "#eed4c7", size: 40},
  ...
  */
  var projections = {
    "airy": {n:"Airyâ€™s Minimum Error", arg:Math.PI/2, scale:360, ratio:1.0, clip:true},
  ...
  //  "butterfly": {n:"Butterfly", arg:null, scale:31, ratio:1.1, clip:true},
    ...
    "larrivee": {n:"l'ArrivÃ©e", arg:null, scale:160, ratio:1.25},
  ...
  */
  ...
  var Trig = {
    ...
    cartesian: function(p) {
      var Ï• = p[0], Î¸ = halfÏ€ - p[1], r = p[2];
      return {"x": r * Math.sin(Î¸) * Math.cos(Ï•), "y": r * Math.sin(Î¸) * Math.sin(Ï•), "z": r * Math.cos(Î¸)};
    },
    spherical: function(p) {
      var r = Math.sqrt(p.x * p.x + p.y * p.y + p.z * p.z),
          Î¸ = Math.atan(p.y / p.x),
          Ï• = Math.acos(p.z / r);
      return  [Î¸ / deg2rad, Ï• / deg2rad, r];
    },
  ...
  /*
      ep = epoch (iso-date)
      N = longitude of the ascending node (deg) Î©
      i = inclination to the refrence plane, default:ecliptic (deg) 
      w = argument of periapsis (deg)  Ï‰
      ...
      W = N + w  = longitude of periapsis Ï–
      L = M + W  = mean longitude
      q = a*(1-e) = periapsis distance
      Q = a*(1+e) = apoapsis distance
      P = 2Ï€ * sqrt(a^3/GM) = orbital period (years)
      T = Epoch_of_M - (M(deg)/360_deg) / P  = time of periapsis
      v = true anomaly (angle between position and periapsis) Î½
      E = eccentric anomaly
  ...
  */
  ...
  var Kepler = function () {
    ...
    function derive() {
      ...
      de.P = Ï„ * Math.sqrt(Math.pow(de.a, 3) / gm) / 365.25;
      de.T = de.jd0 - (de.M / halfÏ€) / de.P;
    ...
    function equatorial(pos) {
      ...
      Îµ = (23.439292 - 0.0130042 * de.cy - 1.667e-7 * de.cy * de.cy + 5.028e-7 * de.cy * de.cy * de.cy) * deg2rad;
      sinÎµ = Math.sin(Îµ);
      cosÎµ = Math.cos(Îµ);
      var o = (id === "lun") ? {x:0, y:0, z:0} : {x:pe.x, y:pe.y, z:pe.z};
      de.xeq = de.x - o.x;
      de.yeq = (de.y - o.y) * cosÎµ - (de.z - o.z) * sinÎµ;
      de.zeq = (de.y - o.y) * sinÎµ + (de.z - o.z) * cosÎµ;
  ...
  */  
    ...
    q.await(function(error) {
      ...
      /*defs.append(":sodipodi:namedview")
       ...
       newdiv.id = 'newDiv';
       newdiv.innerHTML = '<textarea id="svg_element">'+svg.node().outerHTML+'</textarea>';
  ...
  var datetimepicker = function(cfg, callback) {
    var date = new Date(), 
        ...
        tz = [{"âˆ’12:00":-720}, {"âˆ’11:00":-660}, {"âˆ’10:00":-600}, {"âˆ’09:30":-570}, {"âˆ’09:00":-540}, {"âˆ’08:00":-480}, {"âˆ’07:00":-420}, {"âˆ’06:00":-360}, {"âˆ’05:00":-300}, {"âˆ’04:30":-270}, {"âˆ’04:00":-240}, {"âˆ’03:30":-210}, {"âˆ’03:00":-180}, {"âˆ’02:30":-150}, {"âˆ’02:00":-120}, {"âˆ’01:00":-60}, {"Â±00:00":0}, {"+01:00":60}, {"+02:00":120}, {"+03:00":180}, {"+03:30":210}, {"+04:00":240}, {"+04:30":270}, {"+05:00":300}, {"+05:30":330}, {"+05:45":345}, {"+06:00":360}, {"+06:30":390}, {"+07:00":420}, {"+08:00":480}, {"+08:30":510}, {"+08:45":525}, {"+09:00":540}, {"+09:30":570}, {"+10:00":600}, {"+10:30":630}, {"+11:00":660}, {"+12:00":720}, {"+12:45":765}, {"+13:00":780}, {"+14:00":840}],
    ...
    var year = d.getFullYear();
      if (!dtrange || dtrange.length < 1) return [yr - 70, yr + 10];
      
      if (dtrange.length === 1 && isNumber(dtrange[0])) {
        if (dtrange[0] >= 100) return [dtrange[0] - 70, dtrange[0] + 10];
  ...
  function interpolateBetween(a, b) {
    var d = Math.max(-1, Math.min(1, dot(a, b))),
        ...
        Î¸ = Math.acos(s * d),
        sinÎ¸ = Math.sin(Î¸);
    return sinÎ¸ ? function(t) {
      var A = s * Math.sin((1 - t) * Î¸) / sinÎ¸,
          B = Math.sin(t * Î¸) / sinÎ¸;
      return [
        a[0] * A + b[0] * B,
        a[1] * A + b[1] * B,
